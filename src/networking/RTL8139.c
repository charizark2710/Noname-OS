...
char *setup_RTL8139(struct pci_device pci_device)
{
    ...
    uint8_t ethertype[2] = {0x08, 0x06};
    struct Package_Send_Descriptor demo;
    demo.len = 16;
    demo.addr = (char *)kmalloc(demo.len);
    // Construct Ethernet header
    memcpy(demo.addr + 0, dest_mac, 6);
    memcpy(demo.addr + 6, mac_address, 6);
    memcpy(demo.addr + 12, ethertype, 2);

    // Payload
    char *payload = demo.addr + 14;
    payload[0] = 'H';
    payload[1] = 'i';
    rtl_send_package((void *)bar0, demo);
    return mac_address;
}

uint8_t tx_cur = 0;
void rtl_send_package(void *base_address, struct Package_Send_Descriptor package)
{
    ...
    while (insl((long)(base_address + TSD_array[tx_cur])) & 0x01)
    {
    ...
    outl((long)(base_address + TSAD_array[tx_cur]), (long)package.addr);
    ...
    outl((long)(base_address + TSD_array[tx_cur]), package.len & ~(1 << 13));
